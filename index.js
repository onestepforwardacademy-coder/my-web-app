...
// ------------------------------------------------------------------------------
const BOT_TOKEN = process.env.TELEGRAM_TOKEN || "7246241507:AAEEsE-UYpe3bKfTgAlIOFIv5L4pfirqVPo";
...
bot.on("callback_query", async (query) => {
    ...
    if (data === "verify_rug_history") {
        ...
        const sent = await bot.sendMessage(chatId, "🛡️ *Enter the token mint address to check dev rug history:*", { 
            parse_mode: "Markdown",
            reply_markup: { inline_keyboard: [[{ text: "⬅️    BACK    ", callback_data: "back_home" }]] }
        });
        state.lastPromptId = sent.message_id;
                            ...
                            if (userTrades[targetId]) {
                                ...
                                if (idx !== -1) {
                                    ...
                                    if (isStopLoss) {
                                        ...
                                        userStopLossHits[targetId].push({ ...item, time: getTimestamp(), reason: "STOP LOSS" });
                                        await updateStatusMessage(targetId, `🔻 *STOP LOSS TRIGGERED*\nAddr: \`${sellAddr}\``, 15000);
                                    } else {
...
bot.on("message", async (msg) => {
    ...
    if (state.awaitingRugToken) {
        ...
        const pyProc = spawnPython("main.py", [text]);
        let output = "";
