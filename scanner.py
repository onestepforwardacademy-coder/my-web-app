...
def run_selenium_screenshot(
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            page.wait_for_timeout(10000) # Give it 10s to load the table content fully
...
def ocr_extract_pair_symbols(screenshot_path: str) -> List[str]:
    ...
    # Use PSM 6 for sparse text blocks (tables)
    text = pytesseract.image_to_string(img, config='--psm 6')
    lines = text.splitlines()

    pair_symbols: List[str] = []
    # More robust regex for ticker symbols
    for line in lines:
        line = line.strip().upper()
        # Find potential 3-10 character uppercase symbols
        matches = re.findall(r'\b([A-Z0-9]{3,10})\b', line)
        for symbol in matches:
            if symbol not in ["PRICE", "AGE", "VOLUME", "DEX", "SOL", "PUMP", "MARKET"]:
